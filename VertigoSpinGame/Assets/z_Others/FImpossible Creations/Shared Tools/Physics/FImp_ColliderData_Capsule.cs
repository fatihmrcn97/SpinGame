...
namespace FIMSpace
{
    public class FImp_ColliderData_Capsule : FImp_ColliderData_Base
    {
        ...
        public override void RefreshColliderData()
        {
            ...
            if (!FEngineering.VIsSame(previousPosition, Collider.transform.position)) diff = true;
            else
                if (!FEngineering.QIsZero(Collider.transform.rotation * Quaternion.Inverse(previousRotation))) diff = true;
            else
            if (preRadius != Capsule.radius || !FEngineering.VIsSame(previousScale, Collider.transform.lossyScale))
                CalculateCapsuleParameters(Capsule, ref Direction, ref radius, ref scaleFactor);
            ...
            previousScale = Collider.transform.lossyScale;
