...
namespace FIMSpace.FTail
{
    public partial class TailAnimator2
    {
        ...
        void ExpertCurvesEndUpdate()
        {
            ...
            if (UsePosSpeedCurve) lastPosCurvKeys = PosCurve.keys; else if (lastPosCurvKeys != null) { lastPosCurvKeys = null; lastPosSpeeds += 0.001f; }

            lastRotSpeeds = RotationRelevancy;
            if (UseRotSpeedCurve) lastRotCurvKeys = RotCurve.keys; else if (lastRotCurvKeys != null) { lastRotCurvKeys = null; lastRotSpeeds += 0.001f; }

            lastSpringiness = Springiness;
            if (UseSpringCurve) lastSpringCurvKeys = SpringCurve.keys; else if (lastSpringCurvKeys != null) { lastSpringCurvKeys = null; lastSpringiness += 0.001f; }

            lastSlithery = Slithery;
            if (UseSlitheryCurve) lastSlitheryCurvKeys = SlitheryCurve.keys; else if (lastSlitheryCurvKeys != null) { lastSlitheryCurvKeys = null; lastSlithery += 0.001f; }

            lastCurling = Curling;
            if (UseCurlingCurve) lastCurlingCurvKeys = CurlingCurve.keys; else if (lastCurlingCurvKeys != null) { lastCurlingCurvKeys = null; lastCurling += 0.001f; }

            lastSlippery = CollisionSlippery;
            if (UseSlipperyCurve) lastSlipperyCurvKeys = SlipperyCurve.keys; else if (lastSlipperyCurvKeys != null) { lastSlipperyCurvKeys = null; lastSlippery += 0.001f; }

            lastTailAnimatorAmount = TailAnimatorAmount;
            if (UsePartialBlend) lastBlendCurvKeys = BlendCurve.keys; else if (lastBlendCurvKeys != null) { lastBlendCurvKeys = null; lastTailAnimatorAmount += 0.001f; }
        }
        ...
        void Expert_UpdateSlithery()
        {
            if (UseSlitheryCurve)
            {
                if (KeysChanged(SlitheryCurve.keys, lastSlitheryCurvKeys)) //for (int i = 0; i < TailBones.Count; i++) TailBones[i].Slithery = GetValueFromCurve(i, SlitheryCurve);
